...
class TenantMiddleware
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        // --- INICIO AJUSTE PARA WOMPI ---
        // Si no hay origin (como en el caso del webhook), buscamos en el JSON de Wompi
        if (!$origin) {
        ...
        // --- FIN AJUSTE PARA WOMPI ---

        if (!$origin) {
            return response()->json(['error' => 'Origen de peticion no identificado'], 400);
        }

        // 2. Limpiar el protocolo (https://) - Tu código original
        $host = str_replace(['https://', 'http://'], '', $origin);

        // 3. Extraer el primer segmento (el identificador) - Tu código original
